// lib/whatsapp.js
// Helper untuk kirim WhatsApp via Fonnte API (Support Teks & Media/URL)

const FONNTE_TOKEN = process.env.FONNTE_TOKEN;
const FONNTE_URL = "https://api.fonnte.com/send";

/**
 * Kirim pesan WhatsApp via Fonnte (Mendukung Teks & Lampiran Media URL)
 * @param {string} nomor - Nomor WA tujuan (format: 08xxx atau 628xxx)
 * @param {string} pesan - Isi pesan yang akan dikirim
 * @param {string|null} [mediaUrl=null] - URL publik foto/video (opsional)
 * @returns {Promise<{sukses: boolean, data: any}>}
 */
export async function kirimWA(nomor, pesan, mediaUrl = null) {
    if (!FONNTE_TOKEN) {
    ...
    try {
        // Siapkan parameter data yang akan dikirim ke Fonnte
        const payload = {
        ...
        // Jika ada link media (foto/video dari Supabase), masukkan ke parameter 'url' milik Fonnte
        if (mediaUrl) {
...
export async function kirimWABatch(antreanPesan = [], options = {}) {
    ...
    for (let i = 0; i < total; i += batchSize) {
        ...
        const batchPromises = batch.map(async (item) => {
            ...
            try {
                // Mendukung pengiriman media juga pada batch jika item memiliki property mediaUrl
                const res = await kirimWA(item.nomor, item.pesan, item.mediaUrl || null);
                return {
                    ...item,
                    sukses: res.sukses,
                    data: res.data,
